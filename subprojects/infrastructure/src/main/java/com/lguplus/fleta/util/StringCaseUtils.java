...
public class StringCaseUtils {
    ...
    /**
     * 주어진 문자열에 _ 가 포함되어 있으면 snakeCase 로 판단하고 _ 가 없으면 camelCase 로 판단하여 두 케이스에 대해 모두 pascalCase 로 변환합니다.
     */
    ...
    /**
     * 알파벳의 대문자와 소문자가 섞여 있을 경우 camelCase 에 대한 처리를 위한 함수입니다. -> isItCamelCase : 앞 글자만 대문자로 변경되면 pascalCase 가 됨 -> SEQ : 대문자만 있으므로 앞글자는 대문자로 나머지는 소문자 처리함 -> seq : 소문자만 있으므로 앞글자는 대문자로 나머지는 소문자 처리함
     */
