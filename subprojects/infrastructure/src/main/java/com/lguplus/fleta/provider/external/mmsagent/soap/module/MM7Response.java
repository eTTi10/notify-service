...
public class MM7Response extends MM7Message {
    ...
    public void load(Element element) {
        ...
        Element child = body.getChildren().get(0);
        ...
        if ("Fault".equals(child.getName())) {
            ...
            if (element.getNamespace("") != null) {
                child = child.getChild("detail", element.getNamespace("")).getChildren().get(0);
            } else {
                if (child.getChild("detail") != null) {
                    child = child.getChild("detail").getChildren().get(0);
                } else {
                    child = child.getChild("detail", MM7Message.ENVELOPE).getChildren().get(0);
                }
            }
        }
        ...
        //		에러
        //		setStatusCode(Integer.parseInt(child.getChildTextTrim("StatusCode")));
        //		setStatusText(child.getChildTextTrim("StatusText"));
    }
    ...
    public Element save(Element parent) {
        ...
        //		if (statusCode > 0) {
        //			status.addContent(new Element("StatusCode").setText(Integer.toString(statusCode)));
        //			status.addContent(new Element("StatusText").setText(statusText));
        //		}
        //		e.addContent(status);

        //		에러
        //		e.addContent(new Element("StatusCode").setText(Integer.toString(statusCode)));
        //		e.addContent(new Element("StatusText").setText(statusText));
