...
import com.lguplus.fleta.provider.external.mmsagent.soap.module.content.BinaryContent;
import com.lguplus.fleta.provider.external.mmsagent.soap.module.content.ContentType;
import com.lguplus.fleta.provider.external.mmsagent.soap.module.content.SoapContent;
import com.lguplus.fleta.provider.external.mmsagent.soap.module.content.TextContent;
...
import com.lguplus.fleta.provider.external.mmsagent.soap.module.inf.JDOMSupport;
...
public class MM7Message implements JDOMSupport {
    ...
    /**
     * Loads a correct subclass of a MM7 message by looking at a SOAP request and instantiating an object of related class. Handles SOAP with attachments.
     *
     * @param in          input stream to load message from
     * @param contentType of a request. Can be an multipart or text/xml
     * @param ctx         configuration for loading of message
     * @return an MM7Message instance
     * @throws IOException if can't deserialize SOAP message or some other IO problem occurs
     * @throws MM7Error    if SOAP fault is received.
     */
    public static MM7Message load(InputStream in, String contentType, MM7Context ctx) throws IOException, MM7Error {
        ...
        if (content.getParts() != null) {
            String start = ct.getParameter("start");
            if (start != null) {
                if (start.length() == 0) {
    ...
    public static void save(MM7Message mm7, OutputStream out, MM7Context ctx) throws IOException {
        ...
        if (mm7.isMultipart()) {
            ...
            w.write("\r\nContent-ID: <" + mm7.getSoapContentId() + ">");
            w.write("\r\n\r\n");

            w.write("<?xml version=\"1.0\" encoding=\"euc-kr\"?>\r\n");
    ...
    public String getSoapBoundary() {
        if (soapBoundary == null) {
            this.soapBoundary = "NextPart_" + UUID.randomUUID();
        }
        return soapBoundary;
    }
    ...
    public void load(Element element) {
        ...
        if (this.namespace == null) {
            throw new IllegalStateException("can't autodetect MM7 namespace: " + body);
        }
    ...
    public Element save(Element parent) {
        ...
        if (mm7Version != null) {
            e.addContent(new Element("MM7Version").setText(mm7Version));
        }
        return e;
    }
