...
public class PushRequestBodyAnnounceVo {
    ...
    /** 보낼 메시지 */
    @NotBlank(message = "필수 BODY DATA 미존재[msg]", payload = ParameterExceedMaxSizeException.class, groups = Groups.C6.class)
    @JsonProperty("msg")
    private String msg;

    /** 추가할 항목 입력(name!^value) */
    @NotEmpty(message = "필수 BODY DATA 미존재[items]", payload = ParameterExceedMaxSizeException.class, groups = Groups.C7.class)
