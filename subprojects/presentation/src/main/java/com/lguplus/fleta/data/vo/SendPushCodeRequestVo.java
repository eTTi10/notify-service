...
import com.lguplus.fleta.exception.ParameterLengthUnderLimitException;
import com.lguplus.fleta.exception.ParameterTypeMismatchException;
import com.lguplus.fleta.validation.Groups;
import com.lguplus.fleta.validation.Perl5Pattern;
...
@Getter
@GroupSequence({Groups.C1.class, Groups.C2.class, Groups.C3.class, Groups.C4.class, Groups.C5.class, Groups.C6.class, Groups.C7.class, Groups.C8.class, Groups.C9.class, Groups.C10.class, Groups.C11.class, Groups.C12.class, Groups.C13.class, Groups.C14.class, Groups.C15.class, Groups.C16.class, Groups.C17.class, Groups.C18.class, SendPushCodeRequestVo.class})
public class SendPushCodeRequestVo {
    ...
    @Perl5Pattern(regexp = "^\\w+$", message = "잘못된 포맷 형식 전달 및 응답 결과 지원하지 않는 포맷(sa_id 는 영문+숫자만 가능합니다)", payload = ParameterTypeMismatchException.class, groups = Groups.C4.class)
    @ParamAlias("sa_id")
    private String saId;
    ...
    @Perl5Pattern(regexp = "^[\\w\\.]+$", message = "잘못된 포맷 형식 전달 및 응답 결과 지원하지 않는 포맷(stb_mac 의 패턴이 일치하지 않습니다.)", groups = Groups.C8.class) //AS IS 패턴
    @ParamAlias("stb_mac")
    private String stbMac;

    @Perl5Pattern(regexp = "^\\w*$", message = "잘못된 포맷 형식 전달 및 응답 결과 지원하지 않는 포맷(reg_id 는 영문+숫자만 가능합니다)", payload = ParameterTypeMismatchException.class, groups = Groups.C9.class)
    @NotBlank(message = "필수 요청 정보 누락(reg_id 가 Null 혹은 빈값 입니다.)", groups = Groups.C10.class)
    @ParamAlias("reg_id")
    private String regId;

    @Perl5Pattern(regexp = "^\\w*$", message = "잘못된 포맷 형식 전달 및 응답 결과 지원하지 않는 포맷(send_code 는 영문+숫자만 가능합니다)", payload = ParameterTypeMismatchException.class, groups = Groups.C11.class)
    ...
    @Perl5Pattern(regexp = "^\\w*$", message = "잘못된 포맷 형식 전달 및 응답 결과 지원하지 않는 포맷(reg_type 는 영문+숫자만 가능합니다)", payload = ParameterTypeMismatchException.class, groups = Groups.C17.class)
    @NotBlank(message = "필수 요청 정보 누락(reg_type 가 Null 혹은 빈값 입니다.)", groups = Groups.C18.class)
    public String getRegType() {
        return StringUtils.defaultString(regType, "1");
    }
